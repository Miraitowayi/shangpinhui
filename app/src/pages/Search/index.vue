...
export default {
  ...
  // },
  computed: {
    //mapGetters里面的写法：传递的数组，因为getters计算式没有划分模块的
  ...
  methods: {
    //向服务器发送器请求获取search模块数据（根据参数不同返回不同的数据进行展示）
    getData() {
    ...
    removeCategoryName() {
      ...
      //带给服务器的参数可有可无：如果属性值为空的字符串还是会把相应的字段带给服务器，但是你把相应的字段变为undefined,当前这个字段不用带给服务器
  ...
  watch: {
    ...
    $route(newValue, oldValue) {
      ...
      //再次发送ajax请求
      this.getData();
      //每一次请求玩不，应该把响应的1、2、3级分类的ID置空，让他接收下一次的相应1、2、3id
      this.searchParams.category1Id = undefined;
      this.searchParams.category2Id = undefined;
      this.searchParams.category3Id = undefined;
    },
