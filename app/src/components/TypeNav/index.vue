...
export default {
  ...
  methods: {
    ...
    goSearch(event){
      //最好的解决方案：编程式导航+事件委派
      //利用事件委派存在一些问题：1 点击一定是a标签吗 2.如何获取参数【1、2、3级分类的产品的名字和id】
      //存在一些问题：事件委派，是把全部的子节点【h3\dt\dl\em】的事件委派给父亲节点
      //点击a标签的时候，才会进行路由跳转 【怎么确定点击的一定是a标签】
      //存在另一些问题：即使你能确定点击的是a标签，如何分别一级、二级、三级分类的标签


      //第一根问题：把子节点当中a标签，加上自定义属性data-categoryName，其余子节点没有
      let element = event.target
      //获取当前触发这个事件的节点【h3\dt\dl\em】。需要带有data-categoryName这样节点【一定是a标签】
  ...
  mounted() {
    ...
    // this.$store.dispatch("categoryList");   放到App.vue组件里 这样只用发送一次请求
